...
- [ ] Добавить Maven Wrapper: `mvnw`, `mvnw.cmd`, `.mvn/wrapper`.
- [ ] Проверить, что проект запускается командой `./mvnw test`.
- [ ] Добавить/актуализировать `.gitignore` для Java/Maven/IDE.
- [ ] Добавить короткую инструкцию запуска backend в README.

## 1.2. Конфигурация и секреты

- [ ] Убрать реальные/похожие на реальные секреты из `application.yml`.
- [ ] Перевести JWT secret на обязательную env-переменную.
- [ ] Разделить профили `local`, `test`, `prod`.
- [ ] Добавить безопасные local default-значения только для локальной разработки.
- [ ] Проверить, что production-профиль не стартует без обязательных секретов.
...
- [ ] `1.1` Добавить Maven Wrapper и проверить запуск тестов.
- [ ] `1.2` Убрать секреты из конфигурации.
- [ ] `1.3` Закрыть CORS.
